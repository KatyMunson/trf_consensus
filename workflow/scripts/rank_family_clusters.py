...
Outputs, all inside --out-dir:
  ...
  --out-summary: one row per cluster, sorted by rank. Includes
    ...
    (a locus count, not a copy-number-weighted one). Also includes
    mean_percent_match / mean_entropy: TRF's own per-locus quality fields,
    averaged across the same cluster members but WEIGHTED by each locus's
    own copy_number (not a plain per-locus average) — a locus that itself
    represents a large array should count proportionally more toward the
    cluster's average quality than a low-copy-number locus. Feeds the QC
    cross-reference diagnostic (plot_copy_number_qc_diagnostic.py).
"""
import argparse
import os
import random
...
def read_locus_qc(path):
    """Parse a 01_raw_consensus.tsv (extract_by_period.py's output) into
    {id: {"copy_number":..., "percent_match":..., "entropy":...}}, for
    summing/averaging per cluster after peeling."""
    locus_qc = {}
    with open(path) as f:
        header = f.readline().rstrip("\n").split("\t")
        id_col = header.index("id")
        cn_col = header.index("copy_number")
        pm_col = header.index("percent_match")
        ent_col = header.index("entropy")
        for line in f:
            if not line.strip():
                continue
            fields = line.rstrip("\n").split("\t")
            locus_qc[fields[id_col]] = {
                "copy_number": float(fields[cn_col]),
                "percent_match": float(fields[pm_col]),
                "entropy": float(fields[ent_col]),
            }
    return locus_qc


def copy_number_weighted_mean(values_and_weights):
    """sum(value * weight) / sum(weight) — used for mean_percent_match/
    mean_entropy so a locus that itself represents a large array counts
    proportionally more toward the cluster's average quality than a
    low-copy-number locus would under a plain per-locus average."""
...
def main():
    ...
    locus_qc = read_locus_qc(args.in_tsv)
    n_total = len(records)
    ...
    for rank, cluster in enumerate(clusters, 1):
        ...
        try:
            member_qc = [locus_qc[rec_id] for rec_id, _ in cluster]
        ...
        total_copy_number = sum(q["copy_number"] for q in member_qc)
        mean_percent_match = copy_number_weighted_mean(
            [(q["percent_match"], q["copy_number"]) for q in member_qc]
        )
        mean_entropy = copy_number_weighted_mean(
            [(q["entropy"], q["copy_number"]) for q in member_qc]
        )
